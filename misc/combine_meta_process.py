'''
preprocessing code for attentional d-emb with access to all meta info.
'''
...
parser.add_argument("--data_dir", type=str, default="/home/ganesh/data/sosweet/full/main", help="data directory")
parser.add_argument("--feat_dir", type=str, default="/home/ganesh/data/sosweet/full", help="feature directory")
parser.add_argument("--permut", action='store_true', help="randomly shuffle data across time")
parser.add_argument("--dest_dir", type=str, default="/home/ganesh/objects/dywoev/meta_attn", help="dest dir file")
...
metastr2seq, seq2metastr = {}, {}
for mpi, meta_path in enumerate(meta_paths):
  meta_str = meta_path.replace("/", "_")
  metastr2seq[meta_str] = mpi
  seq2metastr[mpi] = meta_str
...
def gen_windows(record):
  ...
  for inp in range(num_inputs):
    ...
    inputs.append([target.item(), np.concatenate([left_context, right_context]), meta_ids])
  return inputs
...
# get all data files
source_files_abs = glob.glob(os.path.join(args.data_dir, 'train', '*'))
cand_files_names = []
...
word2id, id2word, cnt = {}, {}, []
with codecs.open(os.path.join(args.data_dir, 'vocab_25K'), 'r', 'utf-8') as f:
  for word_row in f:
...
tweetid2metaid_list, tweetid2userid = [], {}
with open(os.path.join(args.feat_dir, 'main', 'geo_tweets_full.tsv'), 'r') as f:
  for line in f:
...
for meta_id, meta_type, meta_path in zip(meta_ids, meta_types, meta_paths): 
  tweetid2metaid = {}
  cur_meta_str = '%s_%s_%s'%(meta_id, meta_type, meta_path.replace("/", "_"))
  if meta_type == 'embed':
    embeds, embed_dim = {}, None
    with open(os.path.join(args.feat_dir, 'features', meta_id, meta_path), 'r') as f:
      for line in f:
        idx, user_embed = line.strip().split("\t")
        if not embed_dim:
    ...
    pickle.dump(save_obj, open(os.path.join(dest_full_path, 'data', 'meta_%s.pkl'%cur_meta_str), 'wb'))
  ...
  if meta_type == 'category':
    ...
    with open(os.path.join(args.feat_dir, 'features', meta_id, meta_path), 'r') as f:
      for line in f:
        ...
        idx, meta_category = content[0], content[-1]
        if meta_id == 'tweet':
          tweet_id = idx
    ...
    pickle.dump({'catname2id': catname2id, 'catid2name': catid2name}, open(os.path.join(dest_full_path, 'data', 'meta_%s.pkl'%cur_meta_str), 'wb'))
  if meta_type == 'presence':
    ...
    for tweet_id in tweetid2userid:
      ...
      if user_id in userid2seq:
        tweetid2metaid[tweet_id] = user_id
    pickle.dump({'userid2seq': userid2seq, 'userseq2id': userseq2id}, open(os.path.join(dest_full_path, 'data', 'meta_%s.pkl'%cur_meta_str), 'wb'))
  print('obtained %s features for %d tweets (%.2f percent)'%(cur_meta_str, len(tweetid2metaid), (1.0*len(tweetid2metaid))/len(tweetid2userid)) )
  tweetid2metaid_list.append(tweetid2metaid)
...
for source_file_name_arr in source_files_names:
  ...
  for source_file_name in source_file_name_arr:
    ...
    for cand_folder in cand_folders:
      source_file_abs = os.path.join(args.data_dir, cand_folder, source_file_name)
      input_plain = []
      ...
      with codecs.open(source_file_abs, 'r', 'utf-8', errors='ignore') as file:
        for line in file:
          ...
          tweet_id = items[0]
          cur_meta_id = []
          for tweetid2metaid in tweetid2metaid_list:
            cur_meta_id.append(tweetid2metaid[tweet_id] if tweet_id in tweetid2metaid else None)
...
#dat_stats['meta_ids'] = meta_ids
#dat_stats['meta_types'] = meta_types
#dat_stats['meta_paths'] = meta_paths
dat_stats['metastr2seq'] = metastr2seq
dat_stats['seq2metastr'] = seq2metastr
print(dat_stats)
