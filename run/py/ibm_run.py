...
#obj_folder = "/home/ganesh/objects/dywoev/ibm"
...
num_concurrent_jobs = sys.argv[4]
dry_run = (len(sys.argv) > 5)

out_folder = obj_folder+"/out"
scripts_folder = obj_folder+"/scripts"
master_script_file = scripts_folder+('/%s_%d_%d_%d_%s.sh'%(run_name, job_range[0], job_range[1], len(gpu_ids), num_concurrent_jobs))
...
print("bash "+code_folder+"/run/shell/giant_run.sh "+master_script_file+" "+num_concurrent_jobs)
